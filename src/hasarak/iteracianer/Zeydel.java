...
public class Zeydel extends CoefficientMatrices{
    public Zeydel(Matrix A, Vector b){
        setA(A);
        setB(b);
        setName("Զեյդելի մեթոդ");
    }
    ...
    protected Matrix getQ(){
        ...
        for (int i = 0; i < tmp.rows(); i++) {
            for (int j = 0; j < tmp.columns(); j++) {
                if(i == j){
                    tmp.set(i, j, 1 - (inverseC.multiply(getA())).get(i, j));
                } else
                    tmp.set(i, j, 0 - (inverseC.multiply(getA())).get(i, j));
            }
        }
        return tmp;
    }
    ...
    protected Vector getZ(){
        ...
        for (int i = 0; i < tmp.length(); i++) {
            tmp.set(i, (inverseC.multiply(getB())).get(i));
        }
        return tmp;
    }
}
